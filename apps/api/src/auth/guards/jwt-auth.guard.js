...
var JwtAuthGuard = function () {
    ...
    var JwtAuthGuard = _classThis = /** @class */ (function (_super) {
        ...
        JwtAuthGuard_1.prototype.handleRequest = function (err, user, info, context) {
            if (err || !user) {
                throw (err || new common_1.UnauthorizedException('Token de acceso inválido o expirado'));
            }
            // Verificar si es el super admin
            var adminEmail = process.env.SUPER_ADMIN_EMAIL;
            var adminRole = process.env.SUPER_ADMIN_ROLE;
            if (user.sub === adminEmail) {
                // Asegurar que el super admin tenga los datos correctos
                return __assign(__assign({}, user), { sub: adminEmail, email: adminEmail, accountId: adminEmail, role: adminRole, account: {
                        id: adminEmail,
                        status: 'active',
