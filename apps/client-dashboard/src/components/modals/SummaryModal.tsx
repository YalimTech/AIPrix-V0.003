...
const SummaryModal: React.FC<SummaryModalProps> = ({
  isOpen,
  onClose,
  callData, // eslint-disable-line @typescript-eslint/no-unused-vars
}) => {
      ...
      >
          ...
          {customPrompt ? (
            ...
            <div>
              <textarea
                ...
                className="w-full h-32 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none text-sm text-gray-700 leading-relaxed"
              />
            </div>
          ) : (
            /* Generated Summary */
        ...
        <div className="px-6 py-4 border-t border-gray-200 space-y-3">
          <button
            ...
            disabled={
              isGenerating || (customPrompt && !customPromptText.trim())
            }
            className="w-full px-5 py-2.5 text-sm font-medium text-white bg-gray-400 hover:bg-gray-500 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isGenerating ? "Generating..." : "Generate"}
          </button>
          <button
            onClick={onClose}
            className="w-full px-5 py-2 text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors"
          >
            Close
          </button>
        </div>
