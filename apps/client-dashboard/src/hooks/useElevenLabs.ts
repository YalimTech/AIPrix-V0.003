...
export interface ElevenLabsVoice {
  ...
  languageCapabilities?: {
    native: string[]; // Idiomas nativos de la voz
    supported: string[]; // Idiomas que la voz puede hablar con el modelo multilenguaje
    multilingual: boolean; // Si la voz es multilenguaje
    verifiedLanguages?: any[]; // Todos los idiomas verificados con sus modelos
  };
  isConversationOptimized?: boolean;
  // Campos temporales para debug
...
export const useElevenLabsAgents = () => {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<ElevenLabsAgent[]>(
        endpoints.integrations.elevenlabs.agents.list,
      ),
  });
};

// Hook para obtener un agente específico de ElevenLabs
export const useElevenLabsAgent = (agentId: string | null) => {
  return useQuery({
    queryKey: ["elevenlabs", "agents", agentId],
    queryFn: () => {
      if (!agentId) throw new Error("Agent ID is required");
      return apiClient.get<ElevenLabsAgent>(
        endpoints.integrations.elevenlabs.agents.get(agentId),
      );
    },
    enabled: !!agentId,
  });
...
export const useCreateElevenLabsAgent = () => {
  ...
  return useMutation({
    mutationFn: (agentData: {
      ...
      apiClient.post<ElevenLabsAgent>(
        endpoints.integrations.elevenlabs.agents.create,
        {
          name: agentData.name,
...
export const useUpdateElevenLabsAgent = () => {
  ...
  return useMutation({
    mutationFn: ({
      ...
      apiClient.patch<ElevenLabsAgent>(
        endpoints.integrations.elevenlabs.agents.update(id),
        updateData,
      ),
...
export const useDeleteElevenLabsAgent = () => {
  ...
  return useMutation({
    mutationFn: (agentId: string) =>
      apiClient.delete(
        endpoints.integrations.elevenlabs.agents.delete(agentId),
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["elevenlabs", "agents"] });
    },
...
export const useSyncElevenLabsAgent = () => {
  ...
  return useMutation({
    mutationFn: (data: {
      ...
      accountId: string;
    }) => apiClient.post(endpoints.integrations.elevenlabs.agents.sync, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["elevenlabs", "agents"] });
...
const isNativeForLanguage = (
  ...
  return voice.languageCapabilities.native.some(
    (lang) => lang.toLowerCase() === targetLang.toLowerCase(),
  );
};

const canSpeakLanguage = (
  voice: ElevenLabsVoice,
  targetLang: string,
): boolean => {
  ...
  if (!voice.languageCapabilities) {
    ...
    console.log(
      `✅ Fallback: Asumiendo que ${voice.name} puede hablar ${targetLang} (modelo multilenguaje)`,
    );
    return true;
  }

  const canSpeak =
    voice.languageCapabilities.multilingual ||
    voice.languageCapabilities.native.some(
      (lang) => lang.toLowerCase() === targetLang.toLowerCase(),
    );
  ...
  console.log(
    `   - native languages: ${voice.languageCapabilities.native.join(", ")}`,
  );
...
export const useStartConversation = () => {
  ...
  return useMutation({
    mutationFn: (data: {
      ...
      apiClient.post<ElevenLabsConversation>(
        endpoints.integrations.elevenlabs.conversations.start,
        data,
      ),
    onSuccess: () => {
      queryClient.invalidateQueries({
...
export const useRecentConversations = (limit: number = 10) => {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<ElevenLabsConversation[]>(
        `${endpoints.integrations.elevenlabs.conversations.recent}?limit=${limit}`,
      ),
  });
};
...
export const useElevenLabsAnalytics = (
  ...
  return useQuery({
    ...
    queryFn: () => {
      ...
      return apiClient.get<ElevenLabsAnalytics>(url);
    },
  });
};

// Hook para obtener uso de ElevenLabs por cliente
export const useElevenLabsUsage = (
  accountId: string,
  ...
  return useQuery({
    ...
    queryFn: () => {
      ...
      return apiClient.get<ElevenLabsUsage>(url);
    },
    enabled: !!accountId,
  });
};
...
export const useElevenLabsOutboundCall = () => {
  return useMutation({
    mutationFn: async (data: {
      ...
      console.log(
        `📞 Iniciando llamada outbound: Agente ${data.agentId} -> ${data.toNumber}`,
      );
      ...
      const response = await apiClient.post<{
        ...
        callSid?: string | null;
      }>("/integrations/elevenlabs/twilio/outbound-call", data);

      console.log(
        `✅ Llamada outbound iniciada exitosamente:`,
        response,
      );
...
export const useCreateAgentTest = () => {
  return useMutation({
    mutationFn: async (testData: {
      ...
      const response = await apiClient.post(
        "/integrations/elevenlabs/testing/create-test",
        testData,
      );
      return response;
    },
...
export const useRunAgentTests = () => {
  return useMutation({
    mutationFn: async (data: {
      ...
      const response = await apiClient.post(
        `/integrations/elevenlabs/testing/run-tests/${data.agentId}`,
        {
          tests: data.tests,
...
export const useGetAgentSignedUrl = () => {
  return useMutation({
    mutationFn: ({
      ...
      return apiClient.get<{ signed_url: string }>(
        `${endpoints.integrations.elevenlabs.agents.get(agentId)}/signed-url${params.toString() ? `?${params.toString()}` : ''}`,
      );
    },
  });
};
  ...
  return useMutation({
    mutationFn: ({
      ...
      return apiClient.get<{ token: string }>(
        `${endpoints.integrations.elevenlabs.agents.get(agentId)}/webrtc-token${params.toString() ? `?${params.toString()}` : ''}`,
      );
    },
  });
};
...
export const useGetAgentLink = () => {
  return useMutation({
    mutationFn: (agentId: string) =>
      apiClient.get<{ agent_id: string; token: any | null }>(
        endpoints.integrations.elevenlabs.agents.getLink(agentId),
      ),
  });
};
  ...
  return useMutation({
    mutationFn: ({
      ...
      return apiClient.get<{
        ...
        `${endpoints.integrations.elevenlabs.agents.get(agentId)}/widget${params.toString() ? `?${params.toString()}` : ''}`,
      );
    },
  });
};
  ...
  return useMutation({
    mutationFn: ({
      ...
      return apiClient.upload<{
        ...
        `${endpoints.integrations.elevenlabs.agents.get(agentId)}/avatar`,
        formData,
      );
    },
  });
  ...
  return useMutation({
    mutationFn: ({
      ...
      apiClient.post<{
        ...
        endpoints.integrations.elevenlabs.agents.simulateConversation(agentId),
        data,
      ),
  });
};
  ...
  return useMutation({
    mutationFn: ({
      ...
      apiClient.post<{
        ...
        endpoints.integrations.elevenlabs.agents.calculateLLMUsage(agentId),
        data,
      ),
  });
};
