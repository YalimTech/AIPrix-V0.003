...
export const useCustomVoices = () => {
    return useQuery({
        ...
        queryFn: async () => {
            const response = await apiClient.get("/integrations/elevenlabs/custom-voices");
            return response;
        },
...
export const useCreateCustomVoice = () => {
    ...
    return useMutation({
        mutationFn: (data) => apiClient.post("/integrations/elevenlabs/custom-voices", data),
        onSuccess: () => {
...
export const useUpdateCustomVoice = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: ({ id, ...updateData }) => apiClient.patch(`/integrations/elevenlabs/custom-voices/${id}`, updateData),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["customVoices"] });
        },
    });
...
export const useDeleteCustomVoice = () => {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id) => apiClient.delete(`/integrations/elevenlabs/custom-voices/${id}`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["customVoices"] });
        },
    });
